<?php 
/**
* @version		$Id: native.php 19 2011-01-18 19:13:58Z datahell $
* @package		Elxis
...
class eUTF extends elxisUTF8 {
	...
	static public function substr($str, $offset, $length = 0) {
		...
		if (!preg_match('#'.$Op.$Lp.'#us',$str, $match)) { return ''; }
		return $match[1];
    }
	...
	static public function strtolower($string) {
		...
		$uni = parent::utf8_to_unicode($string);
		if (!$uni) { return false; }
		$cnt = count($uni);
		for ($i=0; $i < $cnt; $i++) {
		...
		return parent::utf8_from_unicode($uni);
	}
	...
	static public function str_pad($input, $length, $padStr = ' ', $type = STR_PAD_RIGHT) {
		$inputLen = self::strlen($input);
		if ($length <= $inputLen) { return $input; }
		$padStrLen = self::strlen($padStr);
		$padLen = $length - $inputLen;
		if ($type == STR_PAD_RIGHT) {
			$repeatTimes = ceil($padLen / $padStrLen);
			return self::substr($input . str_repeat($padStr, $repeatTimes), 0, $length);
		}
		if ($type == STR_PAD_LEFT) {
        	$repeatTimes = ceil($padLen / $padStrLen);
			return self::substr(str_repeat($padStr, $repeatTimes), 0, floor($padLen)) . $input;
		}
		if ($type == STR_PAD_BOTH) {
        	...
        	$paddingLeft = self::substr(str_repeat($padStr, $repeatTimesLeft), 0, $padAmountLeft);
        	$paddingRight = self::substr(str_repeat($padStr, $repeatTimesRight), 0, $padAmountLeft);
        	return $paddingLeft . $input . $paddingRight;
    	}
    	trigger_error('str_pad: Unknown padding type (' . $type . ')',E_USER_ERROR);
	}
	...
	static public function stristr($haystack, $needle, $part = false) {
		...
		if (count($matches) == 2) { return substr($haystack, strlen($matches[1])); }
		return false;
	}
